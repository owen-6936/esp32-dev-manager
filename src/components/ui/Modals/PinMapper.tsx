...
export default function PinMapper({
  ...
  const esp32Pins = Array.from({ length: 48 }, (_, i) => i + 1);
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 h-screen">
      <div className="bg-slate-800 rounded-xl p-6 w-full max-w-4xl border border-white/20 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white">ESP32 S3 Pin Mapper</h3>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {esp32Pins.slice(0, 24).map((pin) => (
                <div key={pin} className="bg-white/10 rounded p-2 text-center">
                  <div className="text-white text-sm font-bold">GPIO{pin}</div>
                  <div className="text-blue-200 text-xs mt-1">Available</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              {(!selectedProject ||
                !projects.find((p) => p.id === selectedProject)?.pinConfig
                  .length) && (
                <p className="text-blue-200 text-sm">
                  No pin assignments yet. Select a project to view
                  configurations.
                </p>
              )}
            </div>
